...
public class PremiseController {
	...
	private final PremiseService premiseServiceImpl;

	@Autowired
	public PremiseController(PremiseService premiseServiceImpl) {
		this.premiseServiceImpl = premiseServiceImpl;
	}

	@PostMapping
	public ResponseEntity<Premise> createPremise(@RequestBody PremiseDTO premiseDTO) {
		Premise createdPremise = premiseServiceImpl.createPremise(premiseDTO);
		return new ResponseEntity<>(createdPremise, HttpStatus.CREATED);
	}

	@GetMapping
	public ResponseEntity<List<Premise>> getAllPremises() {
		List<Premise> premises = premiseServiceImpl.getAllPremises();
		return new ResponseEntity<>(premises, HttpStatus.OK);
	}

	@GetMapping("/{id}")
	public ResponseEntity<Premise> getPremiseById(@PathVariable Long id) {
		Premise premise = premiseServiceImpl.getPremiseById(id);
	...
	public ResponseEntity<Premise> updatePremise(@PathVariable Long id, @RequestBody PremiseDTO premiseDTO) {
		Premise updatedPremise = premiseServiceImpl.updatePremise(id, premiseDTO);
	...
	public ResponseEntity<String> deletePremise(@PathVariable Long id) {
		try {
			premiseServiceImpl.deletePremise(id);
